...
- **Help command** - Display a list of actions and their usages at any time during the game
- **Atmospheric narrative** - Dark, mysterious setting
- **Interesting NPCs** - Lovingly written NPCs, each with a different personality
- **Text decoration** - Highlighted, bolded, and animated text to show intractables, items, new locations, and important phrasing
...
- **UI/UX** - Designed and implemented the text-based user interface, and added summary and highlighting features for clarity
...
- Build tension with gradual body-horror implications
...
- Working on narrative and writing style was difficult by myself since I don't often write much for my games. To help with this, I would periodically send pieces fo the writing to my friends for feedback during the jam period 
- Unfortunately, I de-prioritized sound design too much and wasn't able to plan for creating a short background song like I usually do. Instead, I looked through background music on Itch.io and found [Cosmic Horror Drones and Melodies by Red Means Recording](https://jjbbllkk.itch.io/cosmic-horror-drones-and-melodies).
...
### Command Parser
The `CommandProcessor` takes player input from the front-end using Godot's `UserInput` node. It breaks down the input text into words and pattern matches with the available commands ("help", "offer", "reflect", "go"). It throws an error to the player if they input an empty, incomplete, or unrecognized command. This script also contains the implementation for each command, allowing the UI, inventory, and location systems to work in conjunction. 
...
These custom messages are written in each `Location` node via the Godot editor. This way, each `Location` can all share one script for functionality, but have unique text for descriptions, successful trades, and failed trades.

To handle traveling between adjacent locations, adding items, and adding trades, there is a `LocationManager` script. This script links the `Pathways` between locations by using the root location's `create_pathway_to_location()` method. `Pathway` is a custom Godot `Resource` that keeps track of which `Locations` are linked together and if either end is currently locked (unreachable by the player, allows for one-way pathways). It also defines items and adds trades to each root location using their built in methods `add_item_to_location` and `add_trade_to_location()`.
...
### Trades & Consequences
Trades are when the player swaps a body part for a merchant's item. The specific body part they are requesting is hinted at in the merchant's dialogue when you approach their stall. These trades are defined in the `CommandProcessor` and are triggered by the "offer" command in-game. The player receives unique dialogue based on if the train was a success or a failure. On a success, the merchant's item is removed from the location and added to the player's inventory, while the offered body part is removed from the player.
...
Upon reflection, this game assumes players are familiar with text-based adventure games or using a command line. This is a fair assumption for the targeted audience, other game developers, but other players may not have this experience so onboarding could be improved by adding a small explanation on how to use the commands and include examples for each one.
